...
export function authGuard(req: Request, _res: Response, next: NextFunction) {
  ...
  try {
    ...
    req.user = { id: payload.id, name: payload.name, email: payload.email, role: payload.role, brandId: payload.brandId, brand: payload.brand };
    next();
  } catch {
...
export function scopedBrandId(req: Request, requestedBrandId?: number) {
  ...
  if (req.user.role === 'superadmin') {
    if (!requestedBrandId) throw new HttpError(400, 'Brand wajib dipilih.');
    return requestedBrandId;
  }
  if (!req.user.brandId) throw new HttpError(403, 'Akun belum terhubung ke brand.');
