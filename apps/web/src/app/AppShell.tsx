...
import { BarChart3, BookOpen, Building2, ChevronDown, Inbox, KanbanSquare, LogOut, Menu, PackageOpen, Radio, Settings2, ShieldCheck, Smartphone, Users2, X } from 'lucide-react';
...
/** Status koneksi realtime yang sebenarnya (socket), bukan indikator statis. */
function RealtimeIndicator() {
  const status = useUiStore((state) => state.realtimeStatus);
  const meta = {
    online: { label: 'Realtime aktif', dot: 'bg-emerald-500', ping: true },
    connecting: { label: 'Menghubungkan…', dot: 'bg-amber-500', ping: false },
    offline: { label: 'Realtime terputus', dot: 'bg-rose-500', ping: false },
  }[status];
  return (
    <div
      role="status"
      aria-live="polite"
      title={status === 'offline' ? 'Pembaruan otomatis berhenti. Data akan disinkronkan ulang saat koneksi pulih.' : undefined}
      className="hidden items-center gap-2 rounded-md border border-zinc-200 bg-white px-2.5 py-1 text-xs font-medium text-zinc-600 shadow-xs sm:flex"
    >
      <span className="relative flex h-2 w-2">
        {meta.ping && <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75" />}
      ...
      >
        ...
        <div className="relative flex h-20 shrink-0 items-center justify-between px-4">
          <NavLink to="/" className="flex items-center min-w-0" title="CRM AZHAN">
            ...
            <span className="min-w-0 overflow-hidden transition-all duration-300 block lg:hidden lg:group-hover/sidebar:block opacity-100 max-w-[180px] lg:opacity-0 lg:max-w-0 lg:group-hover/sidebar:opacity-100 lg:group-hover/sidebar:max-w-[180px] ml-3">
              ...
              <span className="block truncate text-[10px] uppercase tracking-[.18em] text-zinc-400">Conversion desk</span>
            </span>
          </NavLink>
        ...
        <nav className="relative flex-1 space-y-2 px-4 py-2 overflow-y-auto thin-scrollbar overflow-x-hidden">
          <div className="block lg:hidden lg:group-hover/sidebar:block pb-1">
            <p className="text-[10px] font-bold uppercase tracking-[.16em] text-zinc-500 whitespace-nowrap px-1">
              Workspace
            </p>
          </div>
          ...
          {/* ADMIN NAV — disembunyikan sementara; hapus `false &&` untuk menampilkan kembali */}
          {false && (user?.role === 'superadmin' || user?.role === 'admin') && (
            <>
              <div className="py-2">
                <div className="block lg:hidden lg:group-hover/sidebar:block pb-1">
                  <p className="text-[10px] font-bold uppercase tracking-[.16em] text-zinc-500 whitespace-nowrap px-1">
              ...
              >
                <Settings2 size={20} strokeWidth={1.9} className="shrink-0" />
                <span
                  className={cn(
                    'truncate whitespace-nowrap transition-all duration-300',
                    'block max-w-[180px] opacity-100',
          ...
          {(user?.role === 'superadmin' || user?.role === 'admin') && (
            <>
              ...
              {[
                ...
                { to: '/brands', label: 'Brand', icon: Building2 },
                { to: '/devices', label: 'Perangkat WA', icon: Smartphone },
            ...
            <DropdownMenu.Trigger asChild>
              ...
              >
                ...
                <span className="min-w-0 flex-1 overflow-hidden transition-all duration-300 block lg:hidden lg:group-hover/sidebar:block opacity-100 max-w-[160px] lg:opacity-0 lg:max-w-0 lg:group-hover/sidebar:opacity-100 lg:group-hover/sidebar:max-w-[160px]">
                  ...
                  <span className="block truncate text-[10px] capitalize text-zinc-500">
            ...
            <DropdownMenu.Portal>
              ...
              >
                ...
                <DropdownMenu.Item
                  ...
                  className="flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-sm text-red-600 outline-none hover:bg-red-50 transition"
                >
          ...
          <header className="sticky top-0 z-20 flex h-14 items-center justify-between gap-3 border-b border-zinc-200/90 bg-white/80 px-4 backdrop-blur-md sm:px-6">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <nav className="flex items-center gap-1.5 min-w-0 text-xs" aria-label="Breadcrumb">
                <NavLink to="/" className="font-medium text-zinc-400 hover:text-zinc-800 transition shrink-0">
                  CRM AZHAN
                </NavLink>
            ...
            <div className="flex items-center gap-2.5 shrink-0">
              {(brands.data?.length ?? 0) > 1 && location.pathname !== '/' && (
                <Select
